...
omARR is an [Omarchy](https://omarchy.org/) bar plugin for homelab people who already run Sonarr, Radarr, Jellyfin, Plex, SABnzbd, or qBittorrent. Click the skull icon to see your *arr fleet on the left and a live content overview on the right. Now playing, on deck, recently added, and calendar. Add custom links and use as a homelab launcher, all without leaving the Omarchy desktop.
...
- Overview: Jellyfin/Plex now playing and on deck, Sonarr/Radarr calendar, SABnzbd and qBittorrent queues
...
Toasts for grabs, imports, Jellyfin/Plex library adds, finished or failed downloads, and services going down or coming back. Click a toast to summon the panel.
...
| Jellyfin | API key + optional profile | Now playing, resume, recently added | Open in browser |
...
For Jellyfin, create an API key under **Dashboard → API Keys**. The optional profile name selects whose resume state appears on deck; leave it blank to use the first enabled profile returned by the server.

Layout (names, URLs, groups, order, notification flags, poll interval, queue page size, density, download progress card) is stored in `~/.config/omarchy/shell.json`. API keys, Jellyfin profile names, Plex tokens, and passwords are stored only in `~/.local/state/omarchy/omarr/credentials.json` (`0600`).

## Icons

Fleet and settings tiles use [Dashboard Icons](https://dashboardicons.com) from [Homarr Labs](https://github.com/homarr-labs/dashboard-icons) (Apache 2.0). The colorful SVGs are bundled under `icons/` so the panel never fetches a CDN at runtime. First-class kind tiles (Sonarr, Radarr, SABnzbd, qBittorrent, Jellyfin, Plex) always get that icon; generic tiles match on the service name.
